...
/*	for (PresetOutputs::cshape_container::iterator pos2 = presetOutputs().customShapes.begin(); 
		pos2 != presetOutputs().customShapes.end(); ++pos2 ) {
		if ((*pos2)->id == (*pos)->id)
...
void MilkdropPreset::initialize_PerPixelMeshes()
{
  ...
  int x,y;
  const PresetInputs& presetInput = presetInputs();
  int gx = presetInput.gx;
  int gy = presetInput.gy;
  const PresetOutputs& presetOutput = presetOutputs();
      for (x=0;x<gx;x++){
	for(y=0;y<gy;y++){
	  _presetOutputs.cx_mesh[x][y]=presetOutput.cx;
	  _presetOutputs.cy_mesh[x][y]=presetOutput.cy;
	  _presetOutputs.sx_mesh[x][y]=presetOutput.sx;
	  _presetOutputs.sy_mesh[x][y]=presetOutput.sy;
	  _presetOutputs.dx_mesh[x][y]=presetOutput.dx;
	  _presetOutputs.dy_mesh[x][y]=presetOutput.dy;
	  _presetOutputs.zoom_mesh[x][y]=presetOutput.zoom;
	  _presetOutputs.zoomexp_mesh[x][y]=presetOutput.zoomexp;
	  _presetOutputs.rot_mesh[x][y]=presetOutput.rot;
	  _presetOutputs.warp_mesh[x][y]=presetOutput.warp;
	}}
}
...
void MilkdropPreset::evalPerPixelEqns()
{
  ...
  int gx = presetInput.gx;
  int gy = presetInput.gy;
  for (int mesh_x = 0; mesh_x < gx; mesh_x++)
	  for (int mesh_y = 0; mesh_y < gy; mesh_y++)
