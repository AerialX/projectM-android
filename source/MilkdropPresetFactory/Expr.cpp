...
//	assert ( item );
	...
	{
		case VAL_T:
			return static_cast<ValExpr*>(item)->eval_val_expr ( mesh_i, mesh_j );
		case PREFUN_T:
			return static_cast<PrefunExpr*>(item)->eval_prefun_expr ( mesh_i, mesh_j );
		case TREE_T:
			return static_cast<TreeExpr*>(item)->eval_tree_expr ( mesh_i, mesh_j );
		default:
			return EVAL_ERROR;
	}
...
GenExpr * GenExpr::param_to_expr ( Param * param )
{
	...
	term.param = param;
	return new GenExpr ( VAL_T, ( void* )new ValExpr(PARAM_TERM_T, &term) );
}
...
GenExpr * GenExpr::prefun_to_expr ( float ( *func_ptr ) ( void * ), GenExpr ** expr_list, int num_args )
{
	...
	return new GenExpr ( PREFUN_T, ( void* ) prefun_expr );
}
...
GenExpr::~GenExpr()
{
	...
	{
		case VAL_T:
			delete static_cast<ValExpr*>( item );
			break;
		case PREFUN_T:
			delete static_cast<PrefunExpr*>( item );
			break;
		case TREE_T:
			delete static_cast<TreeExpr*>( item );
			break;
	}
}
